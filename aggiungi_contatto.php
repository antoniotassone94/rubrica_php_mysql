...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	if(isset($_POST['token']) && isset($_POST['nome']) && isset($_POST['cognome']) && isset($_POST['indirizzo'])){
		...
		//controllo effettuato solo sul server
		$contatto = estrai("contatti",["nome","cognome","indirizzo"],[$nome,$cognome,$indirizzo]);
		if(count($contatto) > 0){
			$controllo = 1;
		...
		if($controllo == 0){
			$risultato = inserisci("contatti",["nome","cognome","indirizzo"],[$nome,$cognome,$indirizzo],"Contatto creato correttamente.<br>");
			if($risultato != null){
				$messaggio .= $risultato;
			}else{
				$controllo = 1;
